...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100 dark:bg-gray-800 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('medications.index')" :active="request()->routeIs('medications.*')" wire:navigate>
                        {{ __('Medications') }}
                    </x-nav-link>
                    ...
                    <x-nav-link :href="route('transfers.index')" :active="request()->routeIs('transfers.*')" wire:navigate>
                        {{ __('Transfer List') }}
                    </x-nav-link>

                    <x-nav-link :href="route('usermgt')" :active="request()->routeIs('usermgt')" wire:navigate>
                        {{ __('User Management') }}
                    </x-nav-link>
                </div>
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('medications.index')" :active="request()->routeIs('medications.*')" wire:navigate>
                {{ __('Medications') }}
            </x-responsive-nav-link>
            ...
            <x-responsive-nav-link :href="route('transfers.index')" :active="request()->routeIs('transfers.*')" wire:navigate>
                {{ __('Transfer List') }}
            </x-responsive-nav-link>

            <x-responsive-nav-link :href="route('usermgt')" :active="request()->routeIs('usermgt')" wire:navigate>
                {{ __('Users Management') }}
            </x-responsive-nav-link>
        </div>
